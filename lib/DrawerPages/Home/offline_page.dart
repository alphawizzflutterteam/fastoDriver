...
import 'package:pristine_andaman_driver/DrawerPages/Rides/ride_info_page.dart';
...
import 'package:pristine_andaman_driver/utils/location_details.dart';
import 'package:pristine_andaman_driver/utils/map.dart';
...
class _OfflinePageState extends State<OfflinePage> with WidgetsBindingObserver {
  ...
  getBookingInfo(tempRefer) async {
    try {
      ...
      if (response['status']) {
        ...
        if (widget.bookingId != '') {
          bookingStatus(model!.id.toString(), "1");
        }
  ...
  getProfile() async {
    try {
      ...
      if (response['status']) {
        ...
        brand = data['car_type'].toString();
        model2 = data['car_model'].toString();
        number = data['car_no'].toString();
        bankName = data['bank_name'].toString();
        accountNumber = data['account_number'].toString();
        code = data['ifsc_code'].toString();
        profileStatus = data['profile_status'].toString();
        isActive = data['is_active'].toString();
        reject = data['reject'].toString();
        print("dta" + data['profile_status'].toString());
        refer = data['referral_code'] ?? '';
        print("imagag ${imagePath + data['insurance'].toString()}");
      } else {
        setSnackbar(response['message'], context);
  ...
  sendSosRequest() async {
    ...
    try {
      ...
      Map response = await apiBase.postAPICall(
          Uri.parse(baseUrl1 + "Payment/driver_sos_contact"), params);
      ...
      bottomNavigationBar: Container(
        child: Column(
          ...
          children: [
            ...
            //                     current != 1
            //                         ? getTranslated(
            //                                 context, Strings.YOU_RE_ONLINE)!
            //                             .toUpperCase()
            //                         : getTranslated(
            //                                 context, Strings.YOU_RE_OFFLINE)!
            //                             .toUpperCase(),
